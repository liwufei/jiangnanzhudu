...
use frontend\api\library\Signature;
...
class Respond
{
	...
	/**
	 * 针对微信映射实际的支付方式代码
	 * 为了方便前端，微信支付在前端都使用CODE=wxpay
	 * 后端再根据访客终端（terminal）判断实际的支付CODE
	 */
	public function getRealPaymentcode($post)
	{
		if ($post->payment_code == 'wxpay' && isset($post->terminal) && !empty($post->terminal)) {
			$terminal = strtoupper($post->terminal);
			if ($terminal == 'APP') {
				return 'wxapppay';
			}
			if ($terminal == 'MP') {
		...
		return $post->payment_code;
	}

	/**
	 * 针对微信映射实际的登录方式代码
	 * 为了方便前端，微信登录在前端都使用CODE=weixin
	 * 后端再根据访客终端（terminal）判断实际的登录CODE
	 */
	public function getRealLogintype($post)
	{
		if ($post->logintype == 'weixin') {
			if (isset($post->terminal) && !empty($post->terminal)) {
				$terminal = strtoupper($post->terminal);
				if ($terminal == 'APP') {
					return 'weixinapp';
				}
