...
class CategoryForm extends Model
{
	...
	public function formData($post = null, $ifpage = false, $querychild = false)
	{
		...
		foreach ($list as $key => $value) {
			$list[$key]['image'] = Formatter::path($value['image'], null, Yii::$app->params['default_image']);

			// 读取下级的时候，不返回ad字段
			if ($querychild) {
				unset($list[$key]['ad']);
			} else $list[$key]['ad'] = Formatter::path($value['ad']);
		}
	...
	public function save($post, $valid = true)
	{
		...
		if (!$this->cate_id || !($model = GcategoryModel::find()->where(['cate_id' => $this->cate_id])->one())) {
		...
		if (isset($post->sort_order)) $model->sort_order = $post->sort_order;
