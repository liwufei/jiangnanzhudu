            ...
            $.getJSON(url([controller+'/child']), {id: id}, function(data){
                if(data.done)
                {
                    ...
                    for(var i = 0; i < res.length; i++)
                    {
						...
						"<td class='align_center' width='10%'><span ectype='inline_edit' fieldname='sort_order' fieldid='"+res[i].cate_id+"' datatype='number' controller='"+controller+"' >"+res[i].sort_order+"<i class='layui-icon layui-icon-edit layui-font-14 layui-font-blue'></i></span></td>"+
						"<td class='align_center' width='10%'>"+if_show+"</td>"+
						"<td class='handler bDiv'><a class='btn blue' href='"+url([controller+'/edit', {id:res[i].cate_id}])+"'><i class='layui-icon layui-icon-edit layui-font-12'></i>"+lang.edit+"</a> <a class='btn red J_AjaxRequest' uri='"+url([controller+'/delete', {id:res[i].cate_id}])+"' confirm='"+lang.drop_confirm+"'><i class='layui-icon layui-icon-close layui-font-12'></i>"+lang.drop+"</a> <a  class='btn green' href='"+url([controller+'/add', {pid:res[i].cate_id}])+"'><i class='layui-icon layui-icon-add-1 layui-font-12'></i>"+lang.add_child+"</a><a href='"+url(['goodsprop/distribute', {cate_id:res[i].cate_id}])+"' class='btn blue'><i class='fa fa-list-ul'></i>"+lang.distribution_prop+"</a></td></tr>";
                    }
		...
		if(data.done) {
    		...
    		for(var i = 0; i < res.length; i++) {
         		var src = "";
          		...
          		if(res[i].add_child) {
        			add_child =  " <a class='btn green' href='"+url([controller+'/add', {pid:res[i].cate_id}])+"'><i class='layui-icon layui-icon-add-1 layui-font-12'></i>"+lang.add_child+"</a>";
				}
				...
				if(res[i].if_show == '1') {
          		...
          		str+="<tr class='"+pid+" row"+id+"'><td class='align_center w30'><input type='checkbox' class='checkitem' value='"+res[i].cate_id+"' /></td>"+
					...
					"<td class='align_center' width='10%'><span ectype='inline_edit' fieldname='sort_order' fieldid='"+res[i].cate_id+"' datatype='number' controller='"+controller+"' >"+res[i].sort_order+"<i class='layui-icon layui-icon-edit layui-font-14 layui-font-blue'></i></span></td>"+
					"<td class='align_center' width='10%'>"+if_show+"</td>"+
					"<td class='handler bDiv'><a class='btn blue' href='"+url([controller+'/edit', {id:res[i].cate_id}])+"'><i class='layui-icon layui-icon-edit layui-font-12'></i>"+lang.edit+"</a> <a class='btn red J_AjaxRequest' confirm='"+lang.drop_confirm+"' uri='"+url([controller+'/delete', {id:res[i].cate_id}])+"'><i class='layui-icon layui-icon-close layui-font-12'></i>"+lang.drop+"</a>" + add_child + "<a href='"+url(['goodsprop/distribute', {cate_id:res[i].cate_id}])+"' class='btn blue'><i class='fa fa-list-ul'></i>"+lang.distribution_prop+"</a></td></tr>";
				}
				pr.after(str);
             	change_background();
            	$('span[ectype="inline_edit"]').unbind('click');
