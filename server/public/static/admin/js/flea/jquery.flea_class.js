            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        ...
                        //排序
                        src += " <span title='可编辑下级分类排序' ajax_branch='goods_class_sort' datatype='number' fieldid='" + data[i].fleaclass_id + "' fieldname='fleaclass_sort' ds_type='inline_edit' class='editable tooltip'>" + data[i].fleaclass_sort + "</span></td>";
                        //名称
                        ...
                        src += " <span title='可编辑下级分类名称' required='1' fieldid='" + data[i].fleaclass_id + "' ajax_branch='goods_class_name' fieldname='fleaclass_name' ds_type='inline_edit' class='editable tooltip'>" + data[i].fleaclass_name + "</span>";
                        //新增下级
                        if (data[i].deep < 3) {
                            src += "<a class='btn-add-nofloat marginleft' href=\"javascript:dsLayerOpen('" + ADMINSITEURL + "/Fleaclass/goods_class_add/fleaclass_parent_id/" + data[i].fleaclass_id + "','新增下级')\"><span>新增下级</span></a>";
                        }
                        src += "</td>";
                        //显示状态
                        src += "<td class='align-center yes-onoff'>";
                        if (data[i].fleaclass_index_show == 0) {
                            src += "<a href='JavaScript:void(0);' class='tooltip disabled' fieldvalue='0' fieldid='" + data[i].fleaclass_id + "' ajax_branch='goods_class_show' fieldname='fleaclass_show' ds_type='inline_edit' title='可编辑该分类是否显示'><img src='" + ADMINSITEROOT + "/images/treetable/transparent.gif'></a>"
                        } else {
                            src += "<a href='JavaScript:void(0);' class='tooltip enabled' fieldvalue='1' fieldid='" + data[i].fleaclass_id + "' ajax_branch='goods_class_show' fieldname='fleaclass_show' ds_type='inline_edit' title='可编辑该分类是否显示'><img src='" + ADMINSITEROOT + "/images/treetable/transparent.gif'></a>"
                        }
                        src += "</td>";
                        //首页显示状态
                        src += "<td class='align-center yes-onoff'>";
                        if (data[i].fleaclass_show == 0) {
                            src += "<a href='JavaScript:void(0);' class='tooltip disabled' fieldvalue='0' fieldid='" + data[i].fleaclass_id + "' ajax_branch='goods_class_index_show' fieldname='fleaclass_index_show' ds_type='inline_edit' title='可编辑该分类是否在首页显示'><img src='" + ADMINSITEROOT + "/images/treetable/transparent.gif'></a>"
                        } else {
                            src += "<a href='JavaScript:void(0);' class='tooltip enabled' fieldvalue='1' fieldid='" + data[i].fleaclass_id + "' ajax_branch='goods_class_index_show' fieldname='fleaclass_index_show' ds_type='inline_edit' title='可编辑该分类是否在首页显示'><img src='" + ADMINSITEROOT + "/images/treetable/transparent.gif'></a>"
                        }
                        src += "</td>";
                        //操作
                        src += "<td class='w84'>";
