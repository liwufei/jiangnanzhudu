...
class Buy
{
    ...
    public function checkFcode($goods_goodid, $fcode)
    {
        $fcode_info = model('goodsfcode')->getGoodsfcode(array(
            'goods_commonid' => $goods_goodid,
            'goodsfcode_code' => $fcode,
            'goodsfcode_state' => 0
        ));
        if ($fcode_info) {
    ...
    private function _createOrderStep6()
    {
        ...
        if (!empty($notice_list)) {
            foreach ($notice_list as $code => $value) {
                $temp = current($value);
                model('cron')->addCron(array(
                    'cron_exetime' => TIMESTAMP,
                    'cron_type' => 'sendStoremsg',
                    'cron_value' => serialize(array(
                        'code' => $code,
                        'store_id' => key($value),
                        'param' => $temp['param'],
                        'weixin_param' => $temp['weixin_param'],
                        'ali_param' => $temp['ali_param'],
                        'ten_param' => $temp['ten_param']
                    )),
                ));
            }
        }
