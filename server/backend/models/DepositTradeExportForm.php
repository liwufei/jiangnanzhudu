...
class DepositTradeExportForm extends Model
{
	...
	public static function download($list)
	{
		...
		foreach ($list as $key => $val) {
			...
			if (($buyer = UserModel::find()->select('username')->where(['userid' => $val['buyer_id']])->one())) {
				$record_value['buyer'] = $buyer->username;
			}

			if ($partyInfo = DepositTradeModel::getPartyInfoByRecord($val['buyer_id'], $val)) {
				$record_value['party'] = $partyInfo['name'];
			}

			$record_value['amount']		= $val['amount'];
			$record_value['status']		= Language::get('TRADE_' . strtoupper($val['status']));
