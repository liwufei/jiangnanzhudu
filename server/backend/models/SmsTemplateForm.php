...
/**
 * @Id SmsTemplateForm.php 2018.8.23 $
 * @author mosir
 */
class SmsTemplateForm extends Model
{
	...
	public function save($post, $valid = true)
	{
		...
		if(!$this->id || !($model = SmsTemplateModel::findOne($this->id))) {
			if(!($model = SmsTemplateModel::find()->where(['code' => $post->code, 'scene' => $post->scene])->one())) {
