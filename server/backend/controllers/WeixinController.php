...
class WeixinController extends \common\base\BaseAdminController
{
	/**
	 * 微信公众号
	 * 自动回复列表 
	...
	public function actionIndex()
	{
		...
		if (!Yii::$app->request->isAjax) {
			...
			$query = WeixinReplyModel::find('reply_id, action, keywords, description')
				->where(['and', ['userid' => 0], ['in', 'action', ['subscribe', 'text', 'keyword']]])
	...
	public function actionSetting()
	{
		if (!Yii::$app->request->isPost) {
			if (!($setting = WeixinSettingModel::find()->where(['code' => 'mp'])->orderBy(['id' => SORT_DESC])->asArray()->one()) || empty($setting['token'])) {
				$setting['token'] = WeixinSettingModel::genToken(32);
			}
			$setting['gatewayUrl'] = Url::toRoute('weixin/notify', Basewind::homeUrl());
			...
			$post = Basewind::trimAll(Yii::$app->request->post(), true, ['autoreg']);
	...
	public function actionMenu()
	{
		...
		$this->params['_foot_tags'] = Resource::import(['script' => 'javascript/treetable/wxtree.js,javascript/inline_edit.js']);
	...
	{
		if (!Yii::$app->request->isPost) {
			$applet = WeixinSettingModel::find()->where(['code' => 'applet'])->orderBy(['id' => SORT_DESC])->asArray()->one();
			$this->params['applet'] = $applet;

			if ($merchant = WeixinSettingModel::find()->where(['code' => 'merapplet'])->orderBy(['id' => SORT_DESC])->asArray()->one()) {
			...
			$post = Basewind::trimAll(Yii::$app->request->post(), true, ['autoreg']);
			$post->code = in_array($post->code, ['applet', 'merapplet']) ? $post->code : 'applet';

			$model = new \backend\models\WeixinSettingForm(['code' => $post->code]);
			if (!$model->save($post, true)) {
				return Message::warning($model->errors);
			}
