...
class ArticleController extends \common\base\BaseAdminController
{
	public function actionIndex()
	{
		...
		if (!Yii::$app->request->isAjax) {
			...
			$query = ArticleModel::find()->alias('a')->select('a.id,a.title,a.cate_id,a.store_id,a.link,a.sort_order,a.if_show,a.add_time,ac.cate_name')
	...
	public function actionEdit()
	{
		...
		if (!$id || !($article = ArticleModel::find()->where(['id' => $id])->asArray()->one())) {
		...
		if (!Yii::$app->request->isPost) {
			...
			$model = new \backend\models\ArticleForm(['id' => $id]);
			if (!($article = $model->save($post, true))) {
				return Message::warning($model->errors);
			}
	...
	public function actionEditcol()
	{
		...
		if (in_array($post->column, ['title', 'if_show', 'sort_order'])) {
			$model = new \backend\models\ArticleForm(['id' => $post->id]);
