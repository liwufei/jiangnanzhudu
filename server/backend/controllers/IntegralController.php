...
class IntegralController extends \common\base\BaseAdminController
{
	/**
	 * 初始化
	 */
	...
	public function actionSetting()
	{
		if(!Yii::$app->request->isPost)
		{
			$sgrades = SgradeModel::find()->select('id,name')->asArray()->all();
			foreach($sgrades as $key => $val) {
				$sgrades[$key]['buygoods'] = IntegralSettingModel::getSysSetting(['buygoods', $val['id']]);
			}
