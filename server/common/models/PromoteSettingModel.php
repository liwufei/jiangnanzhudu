...
/**
 * @Id PromoteSettingModel.php 2018.5.7 $
 * @author mosir
 */

class PromoteSettingModel extends ActiveRecord
{
	/**
	 * @inheritdoc
	 */
	...
	public static function getExclusive($appid, $store_id = 0, $goods_id = 0)
	{
		...
		if (isset($data) && $data && isset($data['status']) && $data['status']) {
			...
			if ($data['rules']['discount']) {
				...
				$desc = sprintf('开启后，新用户首次下单，可享%s折优惠（默认）', $data['discount']);
			} else {
				$data['decrease'] = $data['rules']['decrease'];
				$desc = sprintf('开启后，新用户首次下单，可立减%s元（默认）', $data['decrease']);
			}
		...
		if ($goods_id) {
			$data['selected'] = 0;
			if (($item = PromoteItemModel::getInfo($appid, $store_id, ['goods_id' => $goods_id]))) {
