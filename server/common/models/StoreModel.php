...
class StoreModel extends ActiveRecord
{
	...
	{
		return parent::hasOne(SgradeModel::className(), ['id' => 'sgrade']);
	}
	 ...
	 * 营业中=1，已打烊=0
	 */
	public static function getBusstate($bustime, $store_id = 0)
	{
		if (!$bustime) {
			$bustime = self::find()->select('bustime')->where(['store_id' => $store_id])->scalar();
		}

		if ($bustime) {
			if (is_array($range = explode('~', $bustime))) {
				$time = Timezone::localDate('H:i', true);
				return ($time >= $range[0] && $time < $range[1]) ? 1 : 0;
			}
		}
	...
	public static function computeCredit($credit_value = 0)
	{
		...
		return Resource::getResourceUrl('system/credit/' . $level . '_' . $number . '.gif');
	}
