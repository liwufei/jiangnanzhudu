...
class RecommendGoodsModel extends ActiveRecord
{
	 ...
	 */
	public static function getRecommendGoods($id = 0, $num = 10, $default_image = true, $mall_cate_id = 0, $timeslot = false, $sort_by = false, $cached = true)
	{
		$cache = Yii::$app->cache;
		...
		if ($data === false || !$cached) {
			...
			foreach ($list as $key => $value) {
				...
				$result = $client->getProInfo($value['goods_id'], $value['default_spec']);
				if ($result !== false) {
