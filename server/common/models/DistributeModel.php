...
class DistributeModel extends ActiveRecord
{
	 ...
	 * 运费金额不参与返佣
	 */
	public static function distributeInvite($order = [])
	{
	...
	/**
	 * 获取可用于返佣的金额基数（运费，退款金额不参与返佣）
	 * 支付金额-运费金额-退款金额
	 */
	public static function getDistributeAmount($order = [])
	{
		// 商品90 运费10 优惠-40 积分-10 付款50 退款10 则返佣基数40
		list($realGoodsAmount, $realFreight, $realOrderAmount) = OrderModel::getRealAmount($order['order_id']);
		$amount = $realOrderAmount - $realFreight;

		// 如果有退款，减掉退款金额
		$query = RefundModel::find()->alias('r')->select('r.refund_total_fee')
			->joinWith('depositTrade dt', false)
			->where(['r.buyer_id' => $order['buyer_id'], 'dt.bizOrderId' => $order['order_sn'], 'r.status' => 'SUCCESS'])
			->one();
		if ($query && $query->refund_total_fee > 0) {
			$amount -= $query->refund_total_fee;
		}
	 ...
	 */
	public static function getItemRate($items = [], $distributeAmount)
	{
		$total = 0;
		$each = [];
