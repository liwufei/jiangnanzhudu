...
class TeambuyController extends \common\base\BaseApiController
{
	/**
	 * 获取拼团活动列表
	...
	public function actionList()
	{
		...
		foreach ($list as $key => $value) {
			...
			$list[$key]['teamPrice'] = $this->getTeamPrice($value['spec_id'], $specs, $value['price']);

			if ((count($specs) != count($array = GoodsSpecModel::find()->select('spec_id')->where(['goods_id' => $value['goods_id']])->column()))
				|| (array_diff(array_keys($specs), $array))) {
				TeambuyModel::updateAll(['status' => 0], ['id' => $value['id']]);
				$list[$key]['status'] = 0;
			}
	...
	public function actionRead()
	{
		...
		if (($specs = unserialize($record['specs']))) {
			...
			if ((count($all) != count($array = GoodsSpecModel::find()->select('spec_id')->where(['goods_id' => $record['goods_id']])->column()))
				|| (array_diff(array_keys($specs), $array))
			) {
				TeambuyModel::updateAll(['status' => 0], ['id' => $record['id']]);

				$record['status'] = 0;
				if ($post->status) $record = null;
			}
		}
		if($record) {
	...
	public function actionAdd()
	{
		...
		$client = Plugin::getInstance('promote')->build('teambuy', (object) ['store_id' => Yii::$app->user->id]);
	...
	public function actionUpdate()
	{
		...
		$client = Plugin::getInstance('promote')->build('teambuy', (object) ['store_id' => Yii::$app->user->id, 'id' => $post->id]);
		if (!$client || !$client->save($post, true)) {
			return $respond->output(Respond::CURD_FAIL, $client->errors);
		}
