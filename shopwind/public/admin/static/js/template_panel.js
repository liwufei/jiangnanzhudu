...
$(function () {
    ...
    /* 编辑状态下样式重写 */
    $('body').addClass('editTmplateMode').addClass(__CLIENT__ == 'pc' ? 'pcweb' : '');
    ...
    $('body').on('click', '#template_config .config-close', function () {
        $('#template_config').fadeToggle('slow', 'linear');
    })
...
/**
 * 保存挂件
 * @param boolean reload 保存后重载配置表单
 */
function save_widget(reload) {
...
function save_page() {
    ...
    d.setContents('<div class="center mt20 mb20 f-gray pd10">' + lang.saving + '</div>');
    d.show('center');
    ...
    $.post(url(['template/save', { client: __CLIENT__, page: __PAGE__ }, BACK_URL]), $('#_edit_page_form_').serialize(), function (rzt) {
        var link = '<p class="mt20"><a class="layui-btn layui-bg-blue layui-btn-sm" href="' + rzt.retval.vueurl + '" target="_blank">查看页面</a><p>';
        d.setContents('<div class="center mb20 f-gray pd10">' + rzt.msg + link + '</div>');
    }, 'json');
}
 ...
 * @param {obj} params
 */
function colorRender(dom, callback, params) {
    var input = $(dom).parent('.item').find('input[type="hidden"]');
    layui.use('colorpicker', function () {
        var colorpicker = layui.colorpicker;
        colorpicker.render(Object.assign({
            elem: dom,
            size: 'xs',
            color: input.val(),
...
function config_widget(widget) {
    ...
    $.get(url(['template/config', null, BACK_URL]), { id: _id, name: _name, client: __CLIENT__, page: __PAGE__ }, function (rzt) {
        var _form = '<div class="wraper relative"><div class="widget_config_form"><form widget_id=' + _id + ' widget_name=' + _name + ' enctype="multipart/form-data" method="POST" action="' + url(['template/config', { id: _id, name: _name, client: __CLIENT__, page: __PAGE__ }, BACK_URL]) + '" target="_config_post_iframe_" id="_config_widget_form_"><div class="widget_config_form_body">' + rzt + '</div></form></div><i class="layui-icon layui-icon-close bold config-close pointer"></i></div>';
