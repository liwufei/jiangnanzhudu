...
class LinkPager extends Widget
{
	...
	/**
	 * @var string|bool whether to show total page;
	 * @use as '%sPage'
	 * @author shopwind
	 */
	public $totalPageLabel = false;
	
	/**
	 * @var string|bool whether to show total count;
	 * @use as '%sRecords'
	 * @author shopwind
	 */
	public $totalCountLabel = false;
    ...
    protected function renderPageButtons()
    {
		...
		// @shopwind add conditions for totalCount == 0
        if (($pageCount < 2 && $this->hideOnSinglePage) || ($this->pagination->totalCount == 0)) {
            return '';
        }
		...
		if($this->totalPageLabel) {
			$buttons[] = Html::tag('li', Html::tag("span", sprintf($this->totalPageLabel, $this->pagination->getPageCount())));
		}
 
		// toatl records @shopwind
		if($this->totalCountLabel) {
			$buttons[] = Html::tag('li', Html::tag("span", sprintf($this->totalCountLabel, $this->pagination->totalCount)));
		}
