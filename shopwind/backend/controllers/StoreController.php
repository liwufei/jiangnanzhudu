...
class StoreController extends \common\base\BaseAdminController
{
	public function actionIndex()
	{
		...
		if (!Yii::$app->request->isAjax) {
			...
			foreach ($list as $key => $value) {
				...
				if ($user = UserModel::find()->select('username, phone_mob')->where(['userid' => $value['store_id']])->asArray()->one()) {
	...
	public function actionEdit()
	{
		...
		if (!Yii::$app->request->isPost) {
			...
			$this->params['sgrades'] = SgradeModel::getOptions();
			$this->params['brands'] = BrandModel::find()->select('name')->where(['if_show' => 1])->column();

			$this->params['_foot_tags'] = Resource::import([
				'script' => 'javascript/jquery.ui/jquery.ui.js,javascript/jquery.ui/i18n/' . Yii::$app->language . '.js,javascript/mlselection.js,javascript/jquery.plugins/timepicker/jquery-ui-timepicker-addon.js',
				'style' => 'javascript/jquery.ui/themes/smoothness/jquery.ui.css,javascript/jquery.plugins/timepicker/jquery-ui-timepicker-addon.css'
			]);
