...
class RefundController extends \common\base\BaseAdminController
{
	public function actionIndex()
	{
		...
		if (!Yii::$app->request->isAjax) {
			...
			$query = RefundModel::find()->alias('r')->select('r.refund_id,r.refund_sn,r.tradeNo,r.buyer_id,r.seller_id,r.total_fee,r.refund_total_fee,r.status,r.created,r.finished,r.intervene,r.refund_reason, r.shipped,rb.username as buyer_name,s.store_name,s.store_id')
			...
			foreach ($list as $key => $value) {
				if ($array = DepositTradeModel::find()->select('bizOrderId,payment_code')->where(['tradeNo' => $value['tradeNo']])->asArray()->one()) {
					if ($orderId = OrderModel::find()->select('order_id')->where(['order_sn' => $array['bizOrderId']])->scalar()) {
						$array['items'] = OrderGoodsModel::find()->select('goods_name')->where(['order_id' => $orderId])->column();
					}
	...
	private function getConditions($post, $query = null)
	{
		...
		if ($post->finished_time_from) $post->finished_time_from = Timezone::gmstr2time($post->finished_time_from);
		if ($post->finished_time_to) $post->finished_time_to = Timezone::gmstr2time_end($post->finished_time_to);
		if ($post->finished_time_from && $post->finished_time_to) {
			$query->andWhere(['and', ['>=', 'finished', $post->finished_time_from], ['<=', 'finished', $post->finished_time_to]]);
		}
		if ($post->finished_time_from && (!$post->finished_time_to || ($post->finished_time_to <= $post->finished_time_from))) {
			$query->andWhere(['>=', 'finished', $post->finished_time_from]);
		}
		if (!$post->finished_time_from && ($post->finished_time_to && ($post->finished_time_to > Timezone::gmtime()))) {
			$query->andWhere(['<=', 'finished', $post->finished_time_to]);
		}

		return $query;
	}

	private function checkData($post = null, $realAmount = array(), $refund = [])
	{
