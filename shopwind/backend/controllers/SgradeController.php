...
class SgradeController extends \common\base\BaseAdminController
{
	...
	public function actionEdit()
	{
		...
		if(!$id || !($sgrade = SgradeModel::find()->where(['id' => $id])->one())) {
		...
		else
		{
			...
			$model = new \backend\models\SgradeForm(['id' => $id]);
			if(!($sgrade = $model->save($post, true))) {
				return Message::warning($model->errors);
			}
	...
	public function actionExport()
	{
		...
		$query = SgradeModel::find()->orderBy(['sort_order' => SORT_ASC, 'id' => SORT_DESC]);
		if(!empty($post->id)) {
    ...
    public function actionEditcol()
    {
		...
		if(in_array($post->column, ['need_confirm', 'sort_order'])) {
			...
			$model = new \backend\models\SgradeForm(['id' => $post->id]);
