...
class BrandController extends \common\base\BaseAdminController
{
	public function actionIndex()
	{
		...
		if(!Yii::$app->request->isAjax) 
		{
			...
			$this->params['_foot_tags'] = Resource::import('javascript/inline_edit.js');
			$this->params['page'] = Page::seo(['title' => Language::get('brand_list')]);
			return $this->render('../brand.index.html', $this->params);
		}
		else
		{
			$query = BrandModel::find()->select('id,name,logo,cate_id,tag,letter,sort_order,if_show,recommended')->orderBy(['sort_order' => SORT_ASC, 'id' => SORT_DESC]);
	...
	public function actionEdit()
	{
		...
		if(!$id || !($brand = BrandModel::find()->where(['id' => $id])->one())) {
		...
		else
		{
			...
			$model = new \backend\models\BrandForm(['id' => $id]);
			if(!($brand = $model->save($post, true))) {
				return Message::warning($model->errors);
			}
	...
	public function actionDelete()
	{
		...
		if(is_array($post->id) && !empty($post->id)) {
			BrandModel::deleteAll(['in', 'id', $post->id]);
		}
    ...
    public function actionEditcol()
    {
		...
		if(in_array($post->column, ['name', 'if_show', 'sort_order', 'recommended', 'tag', 'letter'])) {
			$model = new \backend\models\BrandForm(['id' => $post->id]);
	...
	public function actionExport()
	{
		...
		$query = BrandModel::find()->select('id,name,logo,if_show,recommended,tag,letter')->orderBy(['sort_order' => SORT_ASC, 'id' => SORT_DESC]);
		if(!empty($post->id)) {
	...
	private function getConditions($post, $query = null)
	{
		if($query === null) {
			foreach(array_keys(ArrayHelper::toArray($post)) as $field) {
				if(in_array($field, ['name', 'tag', 'letter'])) {
					return true;
				}
			}
			return false;
