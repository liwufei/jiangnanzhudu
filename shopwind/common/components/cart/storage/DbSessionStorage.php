...
class DbSessionStorage implements StorageInterface
{
    ...
    {
		$items = [];
		$carts = CartModel::find()->select('id,userid,gtype,store_id,goods_id,goods_name,spec_id,specification,price,quantity,goods_image,product_id,selected')->where(['userid' => $this->userId])->all();
    ...
    private function saveDb(array $items)
    {
		...
		foreach($items as $cartItem)
		{
			if(($product = $cartItem->getProduct())) 
			{
				...
				$model->save() && $id[] = $model->id;
			}
		}
		
		CartModel::deleteAll(['and', ['userid' => $this->userId], ['not in', 'id', $id]]);
	}
}
