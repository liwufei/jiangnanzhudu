...
class TeambuyOrder extends NormalOrder
{
	...
	{
		$result = [];
		if (!($spec_id = $this->post->spec_id)) {
			return false;
		}

		if (($goods = GoodsSpecModel::find()->alias('gs')->select('gs.spec_id,gs.price,gs.spec_1,gs.spec_2,gs.stock,gs.weight,gs.image,g.goods_id,g.store_id,g.type as gtype,g.goods_name,g.default_image as goods_image,g.spec_name_1,g.spec_name_2')->where(['spec_id' => $spec_id, 'if_show' => 1, 'closed' => 0])->joinWith('goods g', false)->asArray()->one())) {
			...
			$goods['quantity'] = $this->post->quantity > 0 ? $this->post->quantity : 1;
	...
	private function insertTeambuyInfo($order_id, $order_info, $list = array())
	{
		...
		foreach ($list['items'] as $value) {
			if (($teambuy = TeambuyModel::find()->select('id,people')->where(['goods_id' => $value['goods_id'], 'status' => 1])->one())) {
				...
				$model->teamid = $checkTeamid ? $this->post->teamid : $this->getTeamid();
				$model->save();
			}
		}
	}
	...
	{
		// 如果是发起拼单
		if (!isset($this->post->teamid) || empty($this->post->teamid)) {
			return false;
		}

		// 如果已成团
		if (TeambuyLogModel::find()->where(['teamid' => $this->post->teamid, 'status' => 1])->exists()) {
			return false;
		}
		return true;
	}
