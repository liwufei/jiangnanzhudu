...
class Pointcart extends BasePointShop
{
    ...
    public function add()
    {
        ...
        if ($pgid <= 0 || $quantity <= 0) {
            echo json_encode(array('done' => false, 'msg' => lang('pointcart_cart_addcart_fail')));
            die;
        }
        ...
        if (!$data['state']) {
            switch ($data['error']) {
                case 'ParameterError':
                    echo json_encode(array('done' => false, 'msg' => $data['msg'], 'url' => (string)url('Pointprod/plist')));
                    die;
                    break;
                default:
                    echo json_encode(array('done' => false, 'msg' => $data['msg']));
                    die;
                    break;
            }
        }
    ...
    public function drop()
    {
        $pcart_id    = intval(input('get.pc_id'));
        if ($pcart_id <= 0) {
            echo json_encode(array('done' => false, 'msg' => lang('ds_common_del_fail')));
            die;
        }
        $pointcart_model = model('pointcart');
