...
class Rechargecard extends AdminControl
{
    ...
    public function add_card()
    {
        if (!request()->isPost()) {
            ...
            foreach (array_keys($snKeys) as $sn) {
                $snToInsert[] = array(
                    'rc_sn' => $sn,
                    'rc_denomination' => $denomination,
                    'rc_batchflag' => $batchflag,
                    'rc_admin_name' => $adminName,
                    'rc_tscreated' => $ts,
                );
            }
    ...
    private function createExcel($data = array())
    {
        ...
        $excel_obj->setStyle(array(
            'id' => 's_title',
            'Font' => array('FontName' => '宋体', 'Size' => '12', 'Bold' => '1')
        ));
        //header
    ...
    protected function getAdminItemList()
    {
        $menu_array = array(
            array(
                'name' => 'index',
                'text' => lang('ds_list'),
                'url' => (string)url('Rechargecard/index')
            ),
            array(
                'name' => 'add_card',
                'text' => lang('ds_new'),
                'url' => "javascript:dsLayerOpen('" . (string)url('Rechargecard/add_card') . "','" . lang('ds_new') . "')"
            ),
        );
        return $menu_array;
    }
